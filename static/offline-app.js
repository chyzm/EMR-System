(function () {
  ...
  function clearPendingQueue() {
    const queue = getQueue().filter((entry) => entry.status === 'synced');
    saveQueue(queue);
    showToast('Cleared pending queue items.', 'green');
  }
  ...
  function getQueueSummary() {
    ...
    return {
      total: queue.length,
      pending: queue.filter((entry) => entry.status !== 'synced').length,
  ...
  function renderQueueStatus(queue) {
    ...
    const pendingCount = queue.filter((entry) => entry.status !== 'synced').length;
    ...
    const retryButton = failedCount
      ...
      : '';
    const clearSyncedButton = hasSyncedItems
      ...
      : '';
    const clearPendingButton = pendingCount > 0
      ? '<button type="button" data-clear-pending="true" class="rounded border border-yellow-200 px-2 py-1 text-xs font-medium text-yellow-700 hover:bg-yellow-50">Clear pending</button>'
      : '';

    panel.innerHTML = `
      <div class="mb-2 flex items-center justify-between gap-2">
        <div class="text-xs font-semibold uppercase tracking-wide text-gray-500">Queue</div>
        <div class="flex flex-wrap gap-2">${retryButton}${clearSyncedButton}${clearPendingButton}</div>
      </div>
      ${queue.map((entry) => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (queuePanel) {
      queuePanel.addEventListener('click', (event) => {
        ...
        const clearPendingButton = event.target.closest('[data-clear-pending="true"]');
